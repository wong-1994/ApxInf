...
/// Hidden-state form consumed by VLA heads that use Qwen3-VL as a backbone.
/// Kept crate-private so the public Qwen text-generation contract stays small.
pub(crate) struct Qwen3VLBackboneOutput {
    pub features: Tensor,
...
impl GeneralQwen3VL {
    ...
    /// `grid_thw` is `[[T, H, W]]`.
    pub(crate) fn backbone_with_image(
        &mut self,
        token_ids: &[u32],
        pixel_values: &Tensor,
        grid_thw: &[[u32; 3]],
    ) -> Result<Qwen3VLBackboneOutput> {
        ...
        Ok(Qwen3VLBackboneOutput {
            features: x,
            image_mask: token_ids
                .iter()
                .map(|&token| u8::from(token == image_tok))
                .collect(),
    ...
    fn prefill_with_image(
        ...
        let output = self.backbone_with_image(token_ids, pixel_values, grid_thw)?;
        let x = self.backend.rms_norm(
            &output.features,
            &self.weights.output_norm_weight,
            self.config.text.rms_norm_eps,
        )?;
        self.backend.matmul(&x, &self.lm_head)
    }
