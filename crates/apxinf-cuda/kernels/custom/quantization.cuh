...
__global__ void transpose_e4m3_kernel(
    const __nv_fp8_e4m3* input, __nv_fp8_e4m3* output,
    int rows, int cols) {
  __shared__ __nv_fp8_e4m3 tile[32][33];
  int col = blockIdx.x * 32 + threadIdx.x;
  int row = blockIdx.y * 32 + threadIdx.y;
  for (int offset = 0; offset < 32; offset += 8) {
    int source_row = row + offset;
    if (source_row < rows && col < cols) {
      tile[threadIdx.y + offset][threadIdx.x] =
          input[static_cast<int64_t>(source_row) * cols + col];
    }
  }
  __syncthreads();
  ...
  for (int offset = 0; offset < 32; offset += 8) {
    int output_row = row + offset;
    if (output_row < cols && col < rows) {
      output[static_cast<int64_t>(output_row) * rows + col] =
          tile[threadIdx.x][threadIdx.y + offset];
    }
  }
}

// The preceding NNT GEMM applies row_scales[0] * channel_scales[0] through
// its scalar-scale interface. Replace that base scale with the exact dynamic
// row-by-channel product and optionally add BF16 bias in the same pass.
__global__ void rescale_rows_columns_bias_bf16_kernel(
    __nv_bfloat16* values, const float* row_scales,
    const float* channel_scales, const __nv_bfloat16* bias,
    int64_t count, int cols) {
  const float inverse_base =
      1.0f / (row_scales[0] * channel_scales[0]);
  int64_t index = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  const int64_t stride = static_cast<int64_t>(blockDim.x) * gridDim.x;
  for (; index < count; index += stride) {
    ...
    float value = __bfloat162float(values[index]) *
                  row_scales[row] * channel_scales[col] * inverse_base;
    if (bias != nullptr) value += __bfloat162float(bias[col]);
    values[index] = __float2bfloat16(value);
  }
}

__global__ void quantize_columns_f16_e4m3_kernel(
    const half* input, __nv_fp8_e4m3* output, float* scales,
    int rows, int cols) {
  __shared__ float scratch[8];
  ...
  for (int row = threadIdx.x; row < rows; row += blockDim.x) {
    ...
    maximum = fmaxf(maximum, fabsf(__half2float(input[index])));
  }
  const float scale = fmaxf(block_max(maximum, scratch) / 448.0f, 1.0e-12f);
  ...
  for (int row = threadIdx.x; row < rows; row += blockDim.x) {
    ...
    const float value = fminf(
        448.0f, fmaxf(-448.0f, __half2float(input[index]) / scale));
    output[index] = static_cast<__nv_fp8_e4m3>(value);
  }
}
